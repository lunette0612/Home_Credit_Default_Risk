...
print('正在进行类别编码')
train, test = cat_feature_encoder(train, test,encoder='label')
print('类别编码完成！')
...
# 集成模型所需的管道
knn_pipeline, svm_pipeline,GAUSS_pipeline, lr_pipeline = model_pipeline()

# 实例化集成模型
print('训练集成模型')
stacking = model_ensemble(evaluate=False,X=train_reduced,y=target,knn_pipeline=knn_pipeline,new_main_model=new_main_model,GAUSS_pipeline=GAUSS_pipeline,lr_pipeline=lr_pipeline)
