...
# 以LGBM作为主模型，对它进行调参
def lgbm_HPO(X,y):
    main_model = LGBMClassifier(class_weight='balanced',n_estimators=1000,metric='roc_auc',verbose=0,random_state=42)
...
def model_pipeline():
    ...
    return knn_pipeline, svm_pipeline,GAUSS_pipeline, lr_pipeline

# 模型集成
def model_ensemble(X,y,knn_pipeline,new_main_model,GAUSS_pipeline,lr_pipeline,evaluate=True,svm_pipeline=None):
    """
    基学习器：XGB,LGBM,RF，GAUSSIAN,KNN,SVM(未使用)
    元学习器：LR
    :return:stacking
    """
